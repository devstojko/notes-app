  ...
  describe('notes', function() {
    ...
    it('should throw error if extra updates', function () {
      expect(() => {
        ...
        .apply({
          userId: noteOne.userId
        }, [noteOne._id, { title: 'Net title', userAge: '25' }]);
      }).toThrow();
    });
    ...
    it('should not update note if unautheniticated', function () {
      expect(() => {
        Meteor.server.method_handlers['notes.update']
        .apply({}, [noteOne._id])
      }).toThrow();
    ...
    it('should return no notes for user that has none', function () {
      const res = Meteor.server.publish_handlers.notes.apply({ userId: 'noneUser' });
      const notes = res.fetch();
